...
<script>
    	...
    	*/
    	if(db_pw == u_pw){ //비번이 일치했을 때 삭제 프로세스를 태운다.
    		...
    		$.messager.confirm('Confirm','R U SURE?',function(r){
    			if(r){
    				location.href="/board/test.nb?crud=boardDel&bm_no=<%=rMap.get("BM_NO")%>&bs_file=<%=rMap.get("BS_FILE")%>"
    			}
    		});
    	}else{
    		//alert("다르다"); 
	...
	function updateForm(){
		 cmm_window_popup('/board/test.nb?crud=updateForm&bm_no=<%=rMap.get("BM_NO")%>', '600', '400', 'updatdForm')
	/* 	
	...
	function saveForm(){
		$("#plz").textbox('setValue',imsi2);
		$('#f_boardRead').attr("method","post");
		$('#f_boardRead').attr("action","/board/test.nb?crud=boardUpd&imsi="+imsi);
		$('#f_boardRead').submit();
		$('#pan_read').panel('close');
...
<form id="f_boardRead" method="post" enctype="multipart/form-data">
		<input class="easyui-textbox" type="hidden" id="plz" name="imsi2">
...
<!-- =======================================[[상세조회화면 버튼]]===============================-->
		...
		<div id="tb_read" align="center">
			...
			<a href="javascript:updateAction()" class="easyui-linkbutton" iconCls="icon-save"> 저장</a>
			<a href="javascript:self.close()" class="easyui-linkbutton" iconCls="icon-cancel" >닫기</a>
		</div>   
