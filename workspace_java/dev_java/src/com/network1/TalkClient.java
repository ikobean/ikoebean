...
public class TalkClient extends JFrame implements ActionListener, MouseListener{
	public class font extends JComponent{
	...
	public void exitChat() {
		try {
			oos.writeObject(Protocol_.ROOM_OUT+Protocol.seperator+this.nickName);
	...
	/*
	 * public void exit_process() { String exitName; String nickName; String
	 * currentName; for(int i=0; i<dtm_name.getRowCount();i++) {
	 * if(nickName!=currentName) {
	 * 
	 * } } if(nickName!=exitName) { try { oos.writeObject(Protocol_.ROOM_OUT
	 * +Protocol_.seperator+exitName +Protocol_.seperator+"님이 퇴장하였습니다" ); } catch
	 * (Exception e) { // TODO: handle exception } }else { this.dispose(); } }
	 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj==jbtn_font) {
			jdl_font = new JDialog();
			jdl_font.setSize(600,500);
			
			//final JFontChooser jfc_font = new JFontChooser();
		}
