	...
	//메세지보내기
		public void message_process(String msg, String imgChoice) {//메세지 말아야지~ 200|닉네임|메세지
			...
			for(int i=0; i<tc2.wr.jtb_wait.getRowCount();i++) {
				//대화명 비교하므로 getValueAt의 두 번째 파라미터는 0으로 하고
				if(tc2.nickName.equals(tc2.wr.dtm_wait.getValueAt(i, 0))) {//현재 내 대화명과 dtm_wait에서 가져온 대화명이 같니?
					//위에서 같은 대화명을 찾았으므로 i는 위 if문의 i값과 동일한 로우를 가리키고
					//두번째 파라미터는 단톡명이므로 0이 아니라 1을 준다.
					roomTitle = (String) tc2.wr.dtm_wait.getValueAt(i, 1); //방이름 비교 2번
					break;
				}
			}
			//이모티콘 메세지를 전송
