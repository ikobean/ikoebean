...
<script type="text/javascript">
	...
	*/
	function deleteRow(){
		...
		var bsfileArr=[];
		//샌턱한 로우의 값을 담을 변수-배열
		var rows = $("#dg_board").datagrid('getSelections');
		alert("로우 개수 : "+rows.length);
		$.messager.confirm('Confirm','정말 삭제하시겠습니까?',function(r){
			if(r){
				//사용자가 선택한 로우에 대한 Object를 이용하면 field에 접근할 수 있다.
				if(rows==null){
					alert("삭제할 글을 선택하세요.");
					return; //deleterow 함수 탈출
			}
			//선택한 로우의 객체배열을 활용하여 값 로우에 담긴 글번호를 배열 안에 넣기
			//아래 변수는 선택한 로우에서 글 번호는 존재하지만 첨부파일이 없는 경우
			...
			for(var i=0; i<rows.lenth;i++){
				...
				//자바코드에서 StringTokenizer로 문자열을 썰 때 ,를 기준으로 썰어서 담으니까
				//맨 끝에 콤마가 있으면 실제 있는 값보다 1개가 더 생김.
				else{
					vbmnos +=bmnoArr[i]+",";
				}
			}
			...
			//화면상에서만 삭제된다 - 삭제 후 새로 select 처리할 것이므로 의미없다.
			//$("gird_emp").datagrid('deleteRow',getRowIndex(target));
			}/////////////////end of if
		});///////////////////end of confirm
	}/////////////////////////end of deleteform
	...
	function boardList(){//태그 조작(DOM조작)
		...
		//alert(tb_value);
		location.href='/sample/board/boardList.mvc?cb_search='+cb_value
		 +'&keyword='+tb_value
 		 +'&pageNumber=1'
		 +'&pageSize=10' 
	 
		$("#dg_board").datagrid({
			 url:'/sample/board/boardList.mvc?cb_search='+cb_value
					 +'&keyword='+tb_value
			 		 +'&pageNumber=1'
					 +'&pageSize=10' 
			,singleSelect: 'false'
	...
	function pageMove(pageNumber, pageSize){
		...
		$("#dg_board").datagrid({
			url:"/sample/board/boardList.mvc?pageNumber="+pageNumber+"&pageSize="+pageSize
		});
	}
	...
	function delForm(){
		//alert("delForm 호출성공");
		location.href='/sample/board/boardDel.mvc?delAll=delAll'
...
<!-- ===============[[글 목록화면 시작]]==================
 ...
 <table title="글목록" id="dg_board" class="easyui-datagrid" data-options="toolbar:'#tb_board', footer:'#pn_board'"
 	 ...
 	 <thead>
	 	 <tr>
            ...
            <th data-options="field:'BM_WRITER',width:'120px'">작성자</th>
          	...
          	<th data-options="field:'BS_FILE2',width:'200px',hidden:'true'">&nbsp;</th>
	...
	%> 	 
	 	 <tr>
	 	 <!-- 
	 	 header영역에서 hidden으로 처리해서 화면상에는 보이지 않지만 datagrid 안에는 값이 들어있는 상태여야 함.
	 	 그래야 값을 꺼낼 수 있기 때문임.
	 	  ...
	 	  -->
	 	 	...
	 	 	<td>
	 	 	<!-- 너 댓글이니? -->
	 	 	<!-- path ==> JEasyUICommon 소스가 다이렉티브 방식으로 include 돼서 하나로 합쳐지기 때문에 에러가 나지 않음. -->
	<%
...
<a href="javascript:boardDetail(<%=rMap.get("BM_NO")%>)" style="text-decoration:none">
	...
	&nbsp;&nbsp;&nbsp;&nbsp;
 	<input class="easyui-datebox" id="db_date" name="db_date" label="작성일 :" labelAlign="right" style="width:200px;">
        ...
        <a href="javascript:deleteRow()" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" >삭제</a>
    </div>
