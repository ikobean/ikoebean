 ...
 * 대신 setter 객체 주입법과 생성자 객체 주입법으로 객체를 주입받아서 사용하게 됨
 * setter 객체 주입법은 자바 코드로 처리하고
 * 생성자 객체주입법은 xml코드로 처리한다.
 * 스프링 안에는 빈즈들을 관리해주는 클래스가 있음.
 * ApplicationContext와 
 * BeanFactory 두 개의 클래스가 빈관리를 전담하게 됨
 ...
 * 
 * IoC(Inversion of Conversion) Container 임 - 스프링은 
 * 역제어라고 함.
 * 
 * 기존 방식
 * 자바 기반으로 어플리케이션을 개발할 때 자바 객체를 생성하고 서로간의 의존관계를 연결시키는 작업에 대한
 * 제어권은 보통 개발되는 어플리케이션에 있음.
 * 
 * 문제제기
 ...
 * 예) Servlet, EJB
 * 장점 : 컴포넌트간의 결합도가 낮아져 컴포넌트의 재사용 및 확장이 쉽고 
 * 		컴포넌트의 의존관계 변경이  쉬움.
 * 
 ...
 * 
 * 스프링 컨테이너의 유형
 * Spring Container BeanFactory와
 * ApplicationContext라는 두 가지 유형의 컨테이너를 제공함.
 * 
 * BeanFactory
 * 객체를 관리하는 고급 설정 기법임.
 ...
 * 필요한 기능 제공함.
 * XmlBeanFactory
 * 어플리케이션을 구성하는 객체들의 속성 및 참조관계를 XML 기반의 속성 정의 파일에 정의함으로써
 * 컨테이너가 관리할 수 있도록 해줌
...
public class HelloBeanImpl implements HelloBean{
	...
	//xml init-method : bean이 초기화 된 후 호출되는 메소드.
	public void initMethod() {
		System.out.println("init 호출 성공");
}
	//xml destroy-method : bean이 소멸되기 전에 호출되는 메소드.
